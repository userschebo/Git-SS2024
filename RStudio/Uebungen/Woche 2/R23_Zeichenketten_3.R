...
# Für den Test muss die Reihen
Gworte <- grep("ge", heinz, v=TRUE, ignore.case=TRUE)


# A10 ----
# Finde alle Worte, die mit g enden.
worteG <- grep("g$", heinz, v=TRUE)
...
{
  return(grep(^anfang, c("außer", "abc", "nichts", "mit abkürzungen"), v=TRUE))
}
...
{
  abkurzen <- sub("Zitate", "[...]", x=abkuerzen)
}
