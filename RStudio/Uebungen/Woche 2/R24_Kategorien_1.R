...
augenfarbe <- factor(c("grau","grün","blau","braun","grau","blau","grau","grün","grau"))
...
print(tapply(X=einkommen, INDEX=augenfarbe, FUN=function(x) mean(x)))
...
table(person, augenfarbe[3])
  names(which.max(table(augenfarbe)))
...
mean_charlen(einkommen)
mmmmEinkommen <- tapply(X=einkommen, INDEX=augenfarbe, FUN=median)
mmmmEinkommen
# $blau
#   1500   1750   1750   2000 
# $braun
